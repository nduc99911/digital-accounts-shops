      ...
      <main className="mx-auto max-w-6xl px-4 py-4">
        <div className="mb-4 flex flex-wrap items-end justify-between gap-2">
          <div>
            ...
            <div className="mt-1 text-sm text-slate-600">{total} sản phẩm</div>
          </div>
        ...
        <div className="grid gap-4">
          {/* Toolbar */}
          <div className="sticky top-[72px] z-40 rounded-lg bg-white p-3 shadow-sm ring-1 ring-slate-200">
            <form method="get" className="grid gap-3 md:grid-cols-[1fr_220px_auto] md:items-end">
              <div>
                <div className="text-xs font-semibold text-slate-600">Từ khoá trong danh mục</div>
                <input
                  name="q"
                  defaultValue={q}
              ...
              <div className="flex gap-2">
                ...
                <Link
                  href={`/category/${slug}`}
                  className="w-full rounded-md bg-white px-4 py-2 text-center text-sm font-semibold text-slate-700 ring-1 ring-slate-200 hover:bg-slate-50 md:w-auto"
                >
                  Xoá
                </Link>
              </div>
          ...
          {products.length === 0 ? (
            <div className="rounded-lg bg-white p-8 text-center shadow-sm ring-1 ring-slate-200">
              <div className="text-base font-semibold text-slate-900">Không có sản phẩm</div>
              <div className="mt-2 text-sm text-slate-600">Thử đổi từ khoá.</div>
            </div>
          ) : (
