      ...
      <main className="mx-auto max-w-6xl p-4">
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_380px]">
          ...
          <div className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
            ...
            <div className="mt-4">
              <div className="text-3xl font-extrabold text-rose-600">{formatVnd(p.salePriceVnd)}</div>
              <div className="mt-1 flex items-center gap-3">
                <div className="text-sm text-slate-500 line-through">{formatVnd(p.listPriceVnd)}</div>
            ...
            <div className="mt-4 grid gap-2 text-sm">
              <div className="flex items-center justify-between">
                ...
                <span className={inStock ? 'font-semibold text-emerald-700' : 'font-semibold text-rose-700'}>
              ...
              <div className="flex items-center justify-between">
                <span className="text-slate-500">Tồn kho / Đã bán</span>
            ...
            <div className="mt-5 grid gap-2">
              {inStock ? (
                ...
                <button disabled className="w-full rounded-md bg-slate-300 px-4 py-3 text-sm font-semibold text-white">
                  Hết hàng
                </button>
              )}
            ...
            <div className="mt-4 grid gap-2 rounded-lg bg-slate-50 p-4 text-sm">
              ...
              <div className="text-slate-600">• Giao tự động sau khi thanh toán thành công</div>
              <div className="text-slate-600">• Hỗ trợ 1-1 nếu gặp lỗi đăng nhập</div>
              <div className="text-slate-600">• Có thể đổi key khác nếu key lỗi (tuỳ gói)</div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 rounded-xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
          ...
          {p.description ? (
            <div className="prose prose-slate mt-4 max-w-none whitespace-pre-wrap">{p.description}</div>
          ) : (
            <div className="mt-3 text-sm text-slate-500">(Chưa có mô tả chi tiết)</div>
          )}
        </div>
        ...
        {related.length > 0 ? (
          <div className="mt-6">
            <div className="mb-3 flex items-center justify-between">
              ...
              {p.category ? (
                <Link className="text-sm font-semibold text-blue-700 hover:underline" href={`/category/${p.category.slug}`}>
                  Xem thêm
                </Link>
              ) : null}
            </div>
